...
export default function Navbar() {
  ...
  const NavLinkItem = ({ href, children }: { href: string, children: React.ReactNode }) => {
    return (
      <Link href={href} className="relative group no-underline text-zinc-800 text-base py-[5px] transition-colors duration-200">
        <span>{children}</span>
        <span className="absolute bottom-0 left-0 w-full h-[2px] bg-pink-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-out origin-center"></span>
      </Link>
    );
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 h-full flex justify-between items-center">
          <div>
            <Link href="/" className="no-underline">
              <span className="text-3xl sm:text-[28px] font-bold text-zinc-800">Linkcookie</span>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            <span className="text-[28px] text-zinc-800 leading-none">♦</span>
            <span className="text-[15px] text-zinc-800">{clientFormattedDate}</span>
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-zinc-800 to-zinc-300">
            </div>
          </div>
